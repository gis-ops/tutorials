...
-   A running Ubuntu 18.04 with an installed version of Valhalla
...
-   For a more easy approach to setup and run Valhalla, you can alternatively have a look at our [How to setup and run Valhalla on Ubuntu 18.04 with docker](foo).  

---

This tutorial will guide you through the process of how to configure and run Valhalla. This tutorial is the direct successor of the tutorial on [How to build and install Valhalla on Ubuntu 18.04](https://github.com/gis-ops/tutorials/blob/master/valhalla/Valhalla_Install_Ubuntu1804.md). You can follow this tutorial analogous on any Ubuntu 18.04 in a docker or another Virtual Environment with a build and installed version of Valhalla.  
...
The config file is the core part of the Valhalla setup and will hold all necessary file paths, so Valhalla knows where to look for and where to store data. Be sure to follow the instructions correctly:
```bash
...
-   `--mjolnir-tile-extract` defines the file where the tared Valhalla tiles will be stored.
...
This step is optional. The admin areas are used by Valhalla to process calculations whenever they cross administrative areas. As long as you're building a single country it is not necessary. Be sure to include them when you sub regions with multiple countries in it:
```bash
...
-   `sort` is part of the `coreutils` package and will sort the files in `-n` a numerival order.
...
-   `1` defines how many messages Valhalla will yeld into the terminal when first started.  
