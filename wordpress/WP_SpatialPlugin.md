...
- `namespace`: this is the the base URL you want this route to be accessible in, i.e. everything which comes in between `http://localhost:8090/wp-json/` and the `route` name. Here, it's built from the plugin's base `namespace` (next section) plus `/spatial`
...
Create a new file in `wp-content/plugins/gis_ops_spatial` and call it `gis_ops.php` (or whatever you like). This will be the entrypoint of the plugin and call all other PHP classes you define(d):
