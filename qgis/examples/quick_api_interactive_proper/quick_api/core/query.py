...
class Nominatim():
    ...
    def do_request(self, point: QgsPointXY) -> None:
        ...
        self._status_code = response.attribute(QNetworkRequest.HttpStatusCodeAttribute)
    ...
    def get_point(self) -> QgsPointXY:
        return QgsPointXY(float(self._content['lon']), float(self._content['lat']))

    def get_bbox_points(self) -> Tuple[QgsPointXY, QgsPointXY]:
        res_bbox: List[float] = [float(coord) for coord in self._content['boundingbox']]
