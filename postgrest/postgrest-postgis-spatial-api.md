...
In this tutorial you will learn how to build a spatial REST API with the powerful PostgREST library utilizing PostGIS under its hood - in minutes!
We will implement a range of different fancy API endpoints with the following functionality
...
Let's start our docker postgres container on port 5432 (or anything else you prefer).
...
In `pg_hba.conf` you will have to make a small edit to the settings under `Database administrative login by Unix domain socket` (should be on line 85) from `peer` to `trust` and restart the Docker container afterwards.
...
For this tutorial we don't necessarily require a database because we will be implementing logic in PostgreSQL as functions which will spatially compute information on the fly.
...
You should be able to see the `psql` command prompt.
...
Next we should add a role to use for anonymous web requests. 
And when a request comes in, PostgREST will switch into this role in the database to run queries.
...
Now, the `web_anon` role has permission to access functions in the api schema, great!
...
To do that, name the role `authenticator` and also grant him the ability to switch to the `web_anon` role:
...
Let's quit `psql` and try to start our API.

```sh
\q
```

PostgREST optionally can read a configuration file which let's it know where the database is running. 
To this end, go ahead and create a file named `gisops-tutorial.conf` with the following information (remember to adapt the port and password if you have changed it in the earlier steps).
...
Great again, now we are ready to start PostgREST.
...
But, guess what, there is nothing to serve yet.
So let's move on and implement some handy spatial API endpoints!

## Step 2 - Let's return a simple GeoJSON Object

As mentioned earlier, we will implement stored procedures which will calculate spatial information on the fly.
You as a user will have the ability to post `GeoJSON` objects as the payload to the API.
For the sake of demonstration, we will start with a simple procedure which will return the payload without changing it.
...
We will add a simple function to the `api` schema which can look something like this.
It takes a single `JSON` argument and returns it again as `JSON`.
...
It's important to mention here that you can call a function that takes a single parameter of type json by sending the header `Prefer: params=single-object` with your request. 
That way the JSON request body will be used as the single argument.


## Step 3 - Calculating the Length of a LineString

Fantastic, now we can start with some more fun stuff.
Let's go ahead and write a function which is able to calculate the length of a linestring provided in [EPSG:4326](https://spatialreference.org/ref/epsg/wgs-84/).
Note, that the GeoJSON we are providing is in degrees, so we will have to transform to pseudo-mercator (or something similar).
Additionally, we will cast it to `numeric` and after dividing it by 1.000 (to obtain a result in `kilometers`) we will round it to 2 decimals.
...
Voila!
For our route through Kansas, USA it returns:
...
By now you have most likely picked up the gist of how easy it is to implement custom functions.
Similary to the linestring example we could also calculate the area of a polygon.
Let's make this a little trickier and implement it in a way enabling the user to send a `GeoJSON FeatureCollection`.
You will notice that we have to extract the geometry from the feature collection at position 0, so the first element in the list which is our polygon.
...
Last but not least let's implement one more function which will calculate the intersection of 2 polygons.
The function by now should be self-explanatory. 
We just have to make sure we are returning a `GeoJSON` and no longer a numeric value.
...
Which in return will respond with the GeoJSON intersection.
...
By now you will have learnt how to set up and use the PostgREST web API for spatial purposes using PostGIS.
In comparison to other libraries it's probably the most straight forward web framework we have ever stumpled across.
This obviously only touches the surface of what is possible but if you are looking a quick way of setting up an API to handle spatial computations on the fly we can definitely recommend using PostgREST.
Please feel free to get in touch with with us if you have any further questions or ideas for other tutorials on PostgREST!
